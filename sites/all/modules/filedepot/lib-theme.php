...
/*add new columns,--Han*/
$variables['author_names'] = $rec['author_names'];
$variables['doc_content'] = $rec['doc_content'];
...
function template_preprocess_filedepot_filedetail(&$variables) {
  ...
  elseif ($variables['reportmode'] == 'incoming') {
    ...
    $sql  = "SELECT file.author_names, file.cid, file.title, v.fname, file.date, file.version, file.size, ";
  ...
  $A = $query->fetchAssoc();
  if ($A != NULL) {
    list($author_names, $cid, $title, $fname, $date, $cur_version, $size, $description, $submitter, $status, $cur_notes, $tags) = array_values($A);
    $variables['cid'] = $cid;
...
$variables['author_names']  =  filter_xss($author_names);
