...
*/
/* modified by Han
*  $select = "SELECT a.author_names, a.doc_content, a.fid, a.cid, a.title, a.fname, a.date, a.version, a.submitter, "
...
function filedepot_displaySearchListing($query_terms) {
  ...
  if (!empty($query_terms)) {
    ...
    $args = array();
    $fields = array('a.title', 'a.description', 'a.author_names', 'a.doc_content');
    ...
    */
    ...
    foreach ($fields as $field) {
      ...
      foreach ($keywords as $keyword) {
        $where_and .= "LOWER({$field}) like :afield{$field_id}_{$term_id} AND ";
    ...
    */
    ...
    foreach ($fields as $field) {
      ...
      foreach ($keywords as $keyword) {
        ...
        $where_or .= "LOWER({$field}) like :bfield{$field_id}_{$term_id}";
...
function filedepot_displayTagSearchListing($query) {
  ...
  $sql = "SELECT file.author_names, file.doc_content, file.fid as fid,file.cid,file.title,file.fname,file.date,file.version,file.submitter,file.status,";
...
function filedepot_getFileListingSQL($cid, &$out_limit_start, &$out_limit_end) {
  ...
  $sql = "SELECT file.author_names,file.doc_content,file.fid as fid,file.cid,file.title,file.fname,file.date,file.version,file.submitter,file.status,file.fileorder,folderindex.folderprefix,";
...
function filedepotAjaxServer_loadFileDetails() {
  ...
  elseif ($reportmode == 'incoming') {
    ...
    if ($filedepot->checkPermission($cid, 'view') AND db_query("SELECT count(*) FROM {filedepot_files} WHERE fid=:fid",
      ...
      $sql = "SELECT file.author_names, file.doc_content, file.fid,file.cid,file.title,file.description,file.fname,file.date,file.size,file.version,file.submitter,u.name, ";
